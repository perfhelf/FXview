...
- **Engine (后端)**: Python (`godview.py`)
  ...
  - 运行环境: Cloudflare Containers。
  - 定时任务: Cloudflare Worker Cron。
- **Database (数据层)**: Supabase (PostgreSQL)
  - 表: `godview_snapshot`
  - 存储格式: JSONB (`data` 列)
- **Frontend (前端)**: Next.js 14+ (App Router)
  - 托管: Cloudflare OpenNext Worker
  - 渲染: Client Components (`use client` + `useEffect` fetch)
- **Provider (调度层)**: Cloudflare Worker + Container
  - Worker 负责 Cron 与手动触发入口。
  - Container 负责运行现有 Python 引擎并写入 Supabase。
...
**现象**: Provider 运行成功，数据已通过 Upsert 写入，但前端显示的 "Last Update" 依然滞后（落后 8 小时或完全未更新）。
**原因**:
...
### 3.2 Cloudflare Provider 验收
**现象**: 前端页面可访问，但信号数据停留在旧时间。
**检查**:
1. `fxview-provider` Worker 是否可访问 `/health`。
2. Cloudflare Container 状态是否为 `ready`。
3. 手动触发 `/run/godview` 与 `/run/signal-8h` 是否返回 `ok: true`。
4. `GET /api/signal-8h` 是否返回 56 个品种且 `missing_symbols`、`stale_symbols`、`unavailable_symbols` 均为空。
...
### 4.2 Provider 部署
- 前端部署目标: `fxview-xuebz`
- 数据调度目标: `fxview-provider`
- Container app: `fxview-provider-fxviewprovidercontainer`
- 生产域名: `fxview.xuebz.com`
